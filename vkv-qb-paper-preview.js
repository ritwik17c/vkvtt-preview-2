...
// Phase E5.1: formatted preview + scored marks validation, including Answer Any groups. Reads local draft only; no Firestore access/writes.
(function(){
  ...
  function scoring(state){
    ...
    const sections=(state.sections||[]).map(s=>{const countedMarks=(s.questions||[]).reduce((a,q)=>a+(Number(q.marks)||0),0);return{valid:true,attemptAny:0,questionCount:(s.questions||[]).length,countedMarks,availableMarks:countedMarks,problems:[]}});return{valid:true,sections,countedMarks:sections.reduce((a,x)=>a+x.countedMarks,0),availableMarks:sections.reduce((a,x)=>a+x.availableMarks,0),problems:[]};
  }
  function paperHtml(state){
    const score=scoring(state),used=score.countedMarks,target=Number(state.target)||0,balanced=score.valid&&(!target||Math.abs(used-target)<0.001);
    const sections=(state.sections||[]).map((s,si)=>{
      const ss=score.sections[si],sectionMarks=ss.valid?ss.countedMarks:'unresolved';
      ...
      const answerAny=ss.attemptAny?`<div class="inst"><b>Answer any ${ss.attemptAny} of the following ${ss.questionCount} questions.</b></div>`:'';
      return `<section><div class="sec"><strong>Section ${String.fromCharCode(65+si)}${s.name?' — '+esc(s.name):''}</strong><span>${sectionMarks}${ss.valid?' marks':''}</span></div>${s.instructions?`<div class="inst">${nl(s.instructions)}</div>`:''}${answerAny}${qs||'<div class="empty">No questions in this section.</div>'}</section>`;
    }).join('');
    const check=!score.valid?`<div class="check warn">⚠ Scored marks are unresolved. ${score.problems.map(esc).join(' ')}</div>`:target?`<div class="check ${balanced?'ok':'warn'}">${balanced?'✓ Total marks match the target.':`⚠ Draft total is ${used}; target is ${target}. Please correct the marks before final use.`}</div>`:'';
