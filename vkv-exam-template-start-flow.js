(()=>{
  ...
  async function applySubjects(t){
    ...
    f.value=oldF;dispatch(f);search.value=oldS;dispatch(search,'input');return selected;
  }
  ...
  function resetFreshDates(t){
    for(const id of['startDate','endDate','excludedDates','customDates']){const el=$(id);if(el){el.value='';dispatch(el,'input');dispatch(el)}}
    const cadence=$('cadence');if(cadence){cadence.value='custom';dispatch(cadence)}
    for(const b of document.querySelectorAll('[data-exam-date]')){if(b.checked){b.checked=false;dispatch(b)}}
    const s=t.settings||{};const max=$('maxPerDay');if(max&&s.maxExamsPerClassPerDay){max.value=String(s.maxExamsPerClassPerDay);dispatch(max)}
  ...
  function goSetup(name,count){document.querySelector('[data-pane-target="setup"]')?.click();setTimeout(()=>$('startDate')?.parentElement?.scrollIntoView({behavior:'smooth',block:'center'}),120);const m=$('majorTemplateMsg');if(m){m.className='notice success';m.innerHTML=`<b>${name}</b> loaded. ${count} subject selection(s), sessions and reusable settings were imported. Choose a fresh date range and tick the required examination dates; no old dates were carried over.`}}
  async function use(id){
    if(busy)return;busy=true;
    try{
      const {template,name}=await getTemplate(id);if(!confirm(`Create a new editable timetable from “${name}”?\n\nClasses, subjects, sessions and reusable settings will be imported.\n\nThe previous examination dates will NOT be imported; you will choose a fresh date range in Step 1.`))return;
      ...
      resetFreshDates(template);await applySessions(template);const count=await applySubjects(template);applyPrintDetails(template);$('saveDraft')?.click();goSetup(name,count);
