...
function exact(label){const n=norm(label);return $$('button,a,[role="button"]').find(e=>!e.closest('.vkv3sec')&&norm(e.textContent)===n)||$$('button,a,[role="button"]').find(e=>!e.closest('.vkv3sec')&&norm(e.textContent).startsWith(n+' '))||null}
...
function category(){const ntHints=['#officeDutyScheduleBtn','#vkvCard_officeDuty','#officeDutyBtn'].map($).find(Boolean);if(isVisible(ntHints))return'nonTeaching';const tt=$('#myTimetableBtn');if(tt&&!isVisible(tt))return'nonTeaching';const h=norm(document.body.innerText);if(h.includes('non-teaching staff')&& !h.includes('section 2 · teaching staff'))return'nonTeaching';return'teaching'}
...
function build(){css();const anchor=$('#activeScheduleBanner')||$('.activeScheduleBanner')||$('#myAreaGrid');if(!anchor)return false;
 ...
 // S1 inventory — placement only. Existing functional buttons are moved; missing ones receive stable placeholders for later wiring.
 ...
 // Place the three sections as one fixed block immediately after Active Schedule.
 ...
 // Hide only the obsolete containers after their cards have been moved. No output/view containers are touched.
