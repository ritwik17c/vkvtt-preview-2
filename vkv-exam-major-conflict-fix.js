...
function subjectKeys(v){const out=new Set([subjectKey(v)]);for(const part of subjectText(v).split(/\s*(?:\/|&|,|\band\b)\s*/i)){const k=subjectKey(part);if(k)out.add(k)}return out}
...
async function migrateRestoredWorkspace(){
  ...
  const current=visibleCatalogue();if(sameCatalogue(master,current))return;
  const selected=captureSelected();
  ...
  const status=document.getElementById('examSubjectMasterStatus');if(status){status.className='notice info';status.textContent='Saved draft subjects were reconciled with the current Examination Subject Master. Current master names now take precedence over old imported subject labels.'}
  document.dispatchEvent(new CustomEvent('vkv-exam-subject-master-applied'));
}

document.addEventListener('click',e=>{if(e.target.closest?.('[data-open-cloud],[data-revise-cloud]'))setTimeout(()=>migrateRestoredWorkspace(),80)},true);
let n=0,t=setInterval(()=>{if(tidy()||++n>40)clearInterval(t)},250);window.addEventListener('load',()=>setTimeout(tidy,900));
